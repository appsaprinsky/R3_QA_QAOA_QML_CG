...
# =====================================================================
# 2. ROUTE COST EVALUATOR & CLASSICAL LEAF SOLVERS
# =====================================================================
...
# =====================================================================
# 4. RECURSIVE DECOMPOSITION (WITH OPTIONAL 1D PCA PROJECTION)
# =====================================================================
def solve_hierarchical_route(indices, cost_matrix, coords, solver_type="QAOA", max_points=5, use_pca=False):
    """
    Decomposes routes recursively.
    If use_pca=True, reduces 2D coordinates to 1D via PCA prior to k-means clustering.
    """
    n_pts = len(indices)
    if n_pts <= max_points:
    ...
    # Apply 1D PCA if flag is enabled
    if use_pca:
        pca = PCA(n_components=1)
        cluster_input = pca.fit_transform(sub_coords)
    else:
        cluster_input = sub_coords
        
    kmeans = KMeans(n_clusters=k, random_state=42, n_init=10).fit(cluster_input)
    cluster_labels = kmeans.labels_
    
    # Calculate centroids in 2D spatial domain for macro routing
    centroids = np.array([sub_coords[cluster_labels == c].mean(axis=0) for c in range(k)])
    ...
    for c_idx in macro_order:
        ...
        if len(cluster_members) > 0:
            c_route = solve_hierarchical_route(
                cluster_members, cost_matrix, coords, 
                solver_type=solver_type, max_points=max_points, use_pca=use_pca
            )
...
# =====================================================================
# 5. EXECUTION PIPELINE & COMPARISON
# =====================================================================
if __name__ == "__main__":
    ...
    # 2. Standard Standard 2D Clustering (No PCA)
    dijkstra_standard_seq = solve_hierarchical_route(list(range(num_stops)), cost_matrix, coords, solver_type="DIJKSTRA", use_pca=False)
    dijkstra_standard_cost = compute_total_route_cost(dijkstra_standard_seq, cost_matrix)
    
    qaoa_standard_seq = solve_hierarchical_route(list(range(num_stops)), cost_matrix, coords, solver_type="QAOA", use_pca=False)
    qaoa_standard_cost = compute_total_route_cost(qaoa_standard_seq, cost_matrix)
    
    # 3. 1D PCA Reduction + Clustering
    dijkstra_pca_seq = solve_hierarchical_route(list(range(num_stops)), cost_matrix, coords, solver_type="DIJKSTRA", use_pca=True)
    dijkstra_pca_cost = compute_total_route_cost(dijkstra_pca_seq, cost_matrix)
    
    qaoa_pca_seq = solve_hierarchical_route(list(range(num_stops)), cost_matrix, coords, solver_type="QAOA", use_pca=True)
    qaoa_pca_cost = compute_total_route_cost(qaoa_pca_seq, cost_matrix)
    
    # Printable Summary Table
    print("=" * 80)
    print(f"BENCHMARK COMPARISON ON AMAZON ROUTE: {target_route_id[:18]}...")
    print(f"Total Stops: {num_stops}")
    print("=" * 80)
    print(f"{'Method / Configuration':<40} | {'Duration (min)':<15} | {'Gap vs Amazon (%)':<18}")
    print("-" * 80)
    print(f"{'Amazon Planned Route (Ground Truth)':<40} | {amazon_cost:<15.2f} | {'0.00%':<18}")
    print(f"{'2D Hierarchical Dijkstra (No PCA)':<40} | {dijkstra_standard_cost:<15.2f} | {((dijkstra_standard_cost - amazon_cost)/amazon_cost)*100:+.2f}%")
    print(f"{'2D Hierarchical WS-LR-QAOA (No PCA)':<40} | {qaoa_standard_cost:<15.2f} | {((qaoa_standard_cost - amazon_cost)/amazon_cost)*100:+.2f}%")
    print(f"{'1D PCA + Hierarchical Dijkstra':<40} | {dijkstra_pca_cost:<15.2f} | {((dijkstra_pca_cost - amazon_cost)/amazon_cost)*100:+.2f}%")
    print(f"{'1D PCA + Hierarchical WS-LR-QAOA':<40} | {qaoa_pca_cost:<15.2f} | {((qaoa_pca_cost - amazon_cost)/amazon_cost)*100:+.2f}%")
    print("=" * 80)

    # Visualization
    filtered_amz = [i for i in actual_seq if i != depot_idx]
    filtered_q_std = [i for i in qaoa_standard_seq if i != depot_idx]
    filtered_q_pca = [i for i in qaoa_pca_seq if i != depot_idx]
    ...
    # Plot Amazon
    pts_amz = coords[filtered_amz]
    axes[0].plot(pts_amz[:, 1], pts_amz[:, 0], color='#1E88E5', linewidth=1.2)
    axes[0].scatter(delivery_coords[:, 1], delivery_coords[:, 0], c='black', s=20)
    axes[0].set_title(f"Amazon Target\nDuration: {amazon_cost:.1f} min", fontweight='bold')
    axes[0].set_ylabel("Latitude")
    axes[0].grid(True, linestyle=':', alpha=0.6)
    
    # Plot Standard QAOA
    pts_std = coords[filtered_q_std]
    axes[1].plot(pts_std[:, 1], pts_std[:, 0], color='#2E7D32', linewidth=1.2)
    axes[1].scatter(delivery_coords[:, 1], delivery_coords[:, 0], c='black', s=20)
    axes[1].set_title(f"2D QAOA (No PCA)\nDuration: {qaoa_standard_cost:.1f} min", fontweight='bold')
    axes[1].grid(True, linestyle=':', alpha=0.6)
    
    # Plot 1D PCA QAOA
    pts_pca = coords[filtered_q_pca]
    axes[2].plot(pts_pca[:, 1], pts_pca[:, 0], color='#D9381E', linewidth=1.2)
    axes[2].scatter(delivery_coords[:, 1], delivery_coords[:, 0], c='black', s=20)
    axes[2].set_title(f"1D PCA + QAOA\nDuration: {qaoa_pca_cost:.1f} min", fontweight='bold')
    axes[2].grid(True, linestyle=':', alpha=0.6)
    
    plt.suptitle(f"PCA 1D Projection Benchmark Comparison — Route {target_route_id[:18]}...", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.show()