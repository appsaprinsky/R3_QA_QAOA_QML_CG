...
def _concatenate_segments_exact(depot_last_node, segments, matrix, max_k=15):
    ...
    cg_hybrid_bellmanford_sub.solve_bellman_ford_subtour applies here
    too, just with segments (2 orientations each) as the objects being
    ordered instead of raw nodes. Verified against brute force on
    random instances (K up to 8) before use -- see the FIX LOG entry
    this function is documented under for the numbers.

    Cost: O(2^K * K^2) time, exponential in the number of SEGMENTS (not
    nodes) -- typically far fewer than n. Returns (ordered_node_list,
    stitched_cost) for the non-depot segments only.
    """
    K = len(segments)
    if K == 0:
        return [], 0.0
...
def _concatenate_segments_greedy(depot_last_node, segments, matrix):
    ...
    measured to make results WORSE on a real test case -- see the FIX
    LOG entry this is documented under)."""
    tour_nodes = []
